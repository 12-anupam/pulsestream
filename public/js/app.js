...
let polls = [];
let myVotes = {}; // pollId -> optionIndex
...
function setupUI() {
  ...
  if (myRole === 'host') {
    ...
    document.getElementById('stagePlaceholderText').textContent = 'You\'re not live yet — hit ⚡ Go Live to start!';
  } else {
...
socket.on('stream-started', (state) => {
  applyStreamStarted(state);
  if (myRole === 'host') {
...
function applyStreamStarted(state) {
  ...
  uptimeInterval = setInterval(() => {
    ...
    document.getElementById('uptime').textContent = m + ':' + ss;
  }, 1000);
}
...
socket.on('stream-stopped', () => {
  ...
  appendSystemMsg('Stream has ended. Thanks for watching! 🙏');
});
...
function toggleMic() {
  if (rtc.camStream) {
    rtc.camStream.getAudioTracks().forEach(t => { t.enabled = !t.enabled; });
  }
  if (rtc.screenStream) {
    rtc.screenStream.getAudioTracks().forEach(t => { t.enabled = !t.enabled; });
  }
...
async function toggleCam() {
  ...
  if (camEnabled) {
    if (rtc.camStream) {
      rtc.camStream.getTracks().forEach(t => t.stop());
      rtc.camStream = null;
    }
    document.getElementById('camPip').classList.add('hidden');
    ...
    await rtc.updateStreamForAllViewers();
...
socket.on('webrtc-answer', async ({ from, answer }) => {
  if (myRole === 'host') {
    await rtc.handleAnswer(from, answer);
  }
});

socket.on('webrtc-ice', async ({ from, candidate }) => {
  if (myRole === 'host') {
    await rtc.handleIceFromViewer(from, candidate);
  } else {
    await rtc.handleIceFromHost(from, candidate);
  }
});
...
function appendChat(msg) {
  ...
  const isMe = msg.isHost && myRole === 'host';
  ...
  el.innerHTML = `
    <div class="chat-msg-header">
      ...
      <span class="chat-name" style="color:${msg.color}">${escHtml(msg.name)}${msg.isHost ? ' 👑' : ''}</span>
...
function rmOpt(btn) {
  const list = document.getElementById('pollOptionsList');
  if (list.children.length <= 2) { showToast('Need at least 2 options'); return; }
...
function votePoll(pollId, optIdx) {
  if (myVotes[pollId] !== undefined) { showToast('You already voted!'); return; }
...
function renderPolls() {
  ...
  if (!polls.length) {
    c.innerHTML = '<div class="polls-empty">No polls yet</div>';
    return;
  }
  c.innerHTML = polls.map(p => {
    ...
    return `<div class="poll-card">
      <div class="poll-card-q">${escHtml(p.question)}</div>
      ${p.options.map((o, i) => {
        const pct = Math.round(p.votes[i] / total * 100);
        const isLeader = p.votes[i] === maxV && p.votes[i] > 0;
        const myPick = myVotes[p.id] === i;
        return `<div class="poll-opt" onclick="votePoll(${p.id}, ${i})" title="${voted ? 'Already voted' : 'Click to vote'}">
          <div class="poll-opt-header">
            <span class="poll-opt-label">${escHtml(o)}${myPick ? ' ✓' : ''}</span>
            <span class="poll-opt-pct${isLeader ? ' leader' : ''}">${pct}%</span>
          </div>
          <div class="poll-bar-track"><div class="poll-bar-fill${isLeader ? ' leader' : ''}" style="width:${pct}%"></div></div>
        </div>`;
      }).join('')}
